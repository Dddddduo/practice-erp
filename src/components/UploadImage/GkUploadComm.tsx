...
const GkUpload: React.FC<GkUploadProps> = ({value, onChange, fileLength}) => {
  ...
  const handleChange = ev => {
    ...
    const fileID: any = []
    console.log('ev fileList1', ev.fileList)
    ...
    for (const item of ev.fileList) {
      ...
      // });
      // let tmp_map = uploadRef.current
      // let tmp_key = item.uid
      // console.log('uploadRef.current', isEmpty(uploadRef.current), uploadRef.current, item.uid, uploadRef.current[item.uid], tmp_map[tmp_key], tmp_map.tmp_key)
      fileID.push(uploadRef.current[item.uid ?? ''] ?? item.uid)
    }
    console.log('ev fileList2', ev.fileList)
    // console.log('uploadRef.current', result)
