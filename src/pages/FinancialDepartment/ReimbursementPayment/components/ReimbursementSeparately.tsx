...
const ReimbursementSeparately: React.FC<ItemListProps> = ({
  ...
  const columns = [
    {
      ...
      render: (dom, entity) => (
        <DatePicker
          ...
          defaultValue={entity.reim_date ? dayjs(entity.reim_date, 'YYYY-MM-DD') : ''}
        />
      )
    },
    {
  ...
  useEffect(() => {
    console.log('currentItem有什么', currentItem)
  ...
  return (
    <>
      ...
      >
        ...
        <Form.Item>
          <Space>
            {/* 禁用逻辑: 不为'tmp_save','submit', 'reject' 或者currentItem不等于空时  */}
            <Button
              type="primary"
              htmlType="submit"
              disabled={!(includes(['tmp_save', 'submit', 'reject'], currentItem?.status) || isEmpty(currentItem))}
            >
              暂存
            </Button>
            {/* {
