...
import {Button, Drawer, Form, Input, Space, Tag, message, Popconfirm} from "antd";
import {PlusOutlined} from "@ant-design/icons";
import {FormattedMessage, useIntl} from "@@/exports";
import {lockReportStatus, getMaCateList, getMarketList, getShopList} from "@/services/ant-design-pro/report";
...
const ItemList: React.FC<ItemListProps> = ({
  ...
  const columns: ProColumns<API.ReportListItem>[] = [
    ...
    //       <a
    //         onClick={() => {
    //           if (entity.report_tid === 14) {
    //             window.open(`https://erp.zhian-design.com/#/report-pdf?report_id=${entity.id}&report_tid=${entity.report_tid}&report_title=${entity.report_title}`, '_blank')
    //             return
    //           }
        ...
        >
...

          {
            ...
            <Button
              ...
              // target="_blank"
              onClick={() => {
                window.open(`/PDF/ReportPDF?report_id=${record.id}&report_tid=${record.report_tid}&report_title=${record.brand_en + '-' + record.store_cn}店铺电气检查报告`, '_blank')
              }}
            >
              <FormattedMessage id="report.button.pdf" defaultMessage="PDF"/>
            </Button>
  ...
  // handleSearchValuesChange (翻译)=> 控制关闭评估报告的弹窗
  const handleCloseCreateReport = () => {
    setShowCreateReport(false)
    setCurrentItem({})
  }

  // handleDownload  选择报告单进行下载
