import {Button, Card, Form, Input, Modal, Radio, Select, Space, message, Checkbox} from "antd"
...
const CreateReport: React.FC<Props> = ({
  ...
  const handleFinish = (values: any) => {
    ...
    const formatDetail = values.details.map(item => {
      return {
        img_left_file_id: item.img_left_file_id ?? '',
        img_right_file_id: item.img_right_file_id ?? '',
        title: item.title ?? '',
        title_cn:item.title_cn ?? '',
        title_en:item.title_en ?? '',
      }
    })
    console.log('values.details--values.details',values.details,formatDetail)
    let params = {
      ...
      detail_list:
        values.details ? formatDetail : temp10Detail,
  ...
  useEffect(() => {
    ...
    getReportInfo(params).then(res => {
      if (res.success) {
        ...
        // console.log('completion_time: res.data.completed_at---,', res.data);
        form.setFieldsValue({
    ...
    <>
      ...
      >
        ...
        <Form.Item name="synchronous" label="签单同步">
          <Button type="primary" onClick={() => setShowSync(true)}>签单同步</Button>
        </Form.Item>
      ...
      <Modal
        ...
        title="图片列表"
      >
        <UploadFiles value={form.getFieldValue('sign')} />
      </Modal>
    </>
  )
}
