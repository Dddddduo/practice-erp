...
import {Input, Table, Select, InputNumber, Button, DatePicker, Form, Space, Drawer, message} from "antd";
import {PlusCircleOutlined, DeleteOutlined} from "@ant-design/icons";
import {
  getFinanceAloneReimTypeListWithCondition,
  createOrUpdateFinanceReimAlone
...
import {getFinanceReimInfoBatch} from "@/services/ant-design-pro/project";
...
const PaymentEntry: React.FC<ItemListProps> = ({
  ...
  const [originAmount, setOriginAmount] = useState<any>({
    hasAmount: '0.00',
    noHasAmount: '0.00',
    totalAmount: '0.00',
  })
  ...
  const colculateAmount = (data) => {
    ...
    ticketed += Number(originAmount.hasAmount)
    unticketed += Number(originAmount.noHasAmount)
    totals += Number(originAmount.totalAmount)
  ...
  const initData = async () => {
    ...
    if (res.success) {
      setOriginAmount(produce(draft => {
        draft.hasAmount = res.data?.haveAmount ? res.data?.haveAmount : '0.00'
        draft.noHasAmount = res.data?.notHaveAmount ? res.data?.notHaveAmount : '0.00'
        draft.totalAmount = res.data?.pre_reim_amount_total ? res.data?.pre_reim_amount_total : '0.00'
      }))

      setTicketedAmount(res.data?.haveAmount ? res.data?.haveAmount : '0.00')
      setUnticketedAmount(res.data?.notHaveAmount ? res.data?.notHaveAmount : '0.00')
      setTotal(res.data?.pre_reim_amount_total ? res.data?.pre_reim_amount_total : '0.00')
    }
  }
